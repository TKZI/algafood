...
public class FormaPagamentoController {
	...
	public ResponseEntity<List<FormaPagamentoModel>> listar(ServletWebRequest request) {
		...
		String eTag = "0";
		OffsetDateTime dataAtualizacao = formaPagamentoRepository.getDataUltimaDataAtualizacao();
		...
		List<FormaPagamentoModel> formasPagamentoModel = formaPagamentoAssembler.toCollectionObject(formaPagamento);
	...
	public ResponseEntity<FormaPagamentoModel> buscar(@PathVariable Long formaPagamentoId, ServletWebRequest request) {
		ShallowEtagHeaderFilter.disableContentCaching(request.getRequest());
		String eTag = "0";
		OffsetDateTime dataAtualizacao = formaPagamentoRepository.getUltimaDataAtualizacaoById(formaPagamentoId);
		...
		return ResponseEntity.ok().cacheControl(CacheControl.maxAge(10, TimeUnit.SECONDS).cachePublic()).eTag(eTag)
				.body(formaPagamentoAssembler.toModel(formaPagamento));
	}
